...
var model = {
    ...
    getProjectReport: function (data, callback) {
        ...
        async.parallel({
            ...
            totalProjectsPerComponent: function (callback) {
                var newPipeLine = _.cloneDeep(pipeLine);
                newPipeLine.push({
                    $group: {                                     // to calculate total count of filtered data
                        "_id": {
                            ...
                            pab: "$pab_data.name",
                            "totalProjectsPerComponent": {
                                $sum: 1
                            }
                        },
                    }
                });
                Project.aggregate(newPipeLine, function (err, tppc) {
                    if (err) {
                        callback(null, err);
                    } else {
                        if (_.isEmpty(tppc)) {
            ...
            totalDelayedProjectsPerComponent: function (callback) {
                ...
                newPipeLine.push({
                    $match: {
                        "dueDate": { "$lt": new Date() }  // to filter delayed data
                    }
                });
